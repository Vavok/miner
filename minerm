...
giver_address = 'EQAh0X4RydrmGRO0vDvDkWi_ymIO0a5HlxzOp_-fuhgUea-m'#1000
...
def refresh_tonapi(tonapi):
    ...
    while True:
        try:
            b = tonapi.blockchain.execute_get_method(giver_address, 'get_mining_status')
            #num_hex = b.stack[2].num
            seed = int(b.stack[2].num, 16)

            complexity = int(b.stack[0].num, 16)
            iterations = int(b.stack[1].num, 16)
            left= int(b.stack[6].num, 16)
            #print('left',left)

           # print(seed,complexity,iterations)
            time.sleep(3)
...
async def get_pow_params(giver_address: str) -> tuple[int, int, int]:
    ...
    return response[2], response[0], response[1]  # seed complexity iterations
    ...
    try:
       ...
       # path_to_exe = 'pow-miner-cuda'
        ...
        while miner_on:
            ...
            for g in range(n):
                cmd = f'{path_to_exe} -vv -g {g} -F 128 -t {test_time} {wallet_address} {seed_mine} {complexity_miner} 9999999999999999 {giver_address} {path}'
                p = Popen(
                    cmd,
                    stdout=PIPE,
                    stderr=STDOUT,
